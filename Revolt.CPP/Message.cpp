...
*/
RevoltCPP::Message::Message(const json& data, const std::string& token) : RevoltCPP::DiscordObject(token) {
...
*/
RevoltCPP::Message::Message(const Message& old) {
    _token = old._token;
    id = old.id;
...
*/
RevoltCPP::Message RevoltCPP::Message::edit(const std::string& content) {
...
void RevoltCPP::Message::delete_msg() {
    std::string url = "/channels/" + channel->id + "/messages/" + id;
    api_call(url, "DEL");
}
