...
void RevoltCPP::Gateway::start_heartbeating() {
...
void RevoltCPP::Gateway::on_websocket_disconnnect() {
...
RevoltCPP::Gateway::Gateway(const std::string& token, const std::shared_ptr<Threadpool>& threadpool)
    : threadpool(threadpool), io_context(), ssl_context{ssl::context::tlsv12_client} {
    _log = Logger("Revolt.Gateway");
...
RevoltCPP::Gateway::~Gateway() {
    _keepalive = false;
    _heartbeat_task.join();
}

void RevoltCPP::Gateway::set_message_handler(
    const std::function<void(json)>& handler) {
    _message_handler = handler;
}

std::shared_future<void> RevoltCPP::Gateway::connect(const std::string& url) {
...
std::shared_future<void> RevoltCPP::Gateway::send(const json& message) {
...
std::shared_future<void> RevoltCPP::Gateway::close() {
    _keepalive = false;
    _connected = false;
