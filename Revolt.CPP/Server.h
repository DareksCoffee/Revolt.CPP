...
class Server : public RevoltObject {
   public:
    ///the guild's name
    std::string name;
    ...
    DLL_EXPORT Server(Revolt* client, const json& data, const std::string& token);
    DLL_EXPORT Server(Revolt* client, const std::string& id, const std::string& token);
    DLL_EXPORT Server(const Server& old);
    DLL_EXPORT Server(){};
    DLL_EXPORT ~Server();
