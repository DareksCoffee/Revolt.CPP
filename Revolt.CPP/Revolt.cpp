...
*/
RevoltCPP::Revolt::Revolt(const std::string& token, const Intents& intents, const unsigned int num_shards)
    : RevoltObject(token), _num_shards(num_shards) {
    id = "0";
    log = Logger("revolt");
...
/**	Creates a Revolt instance with ONE shard
    @param[in]	token		Bot token for authentication
    @param[in]	intents		Intents used for the gateway.
...
*/
RevoltCPP::Revolt::Revolt(const std::string& token, const Intents& intents, const unsigned int shard_id, const unsigned int num_shards)
    : RevoltObject(token), _num_shards(num_shards) {
...
void RevoltCPP::Revolt::start() {
    srand((unsigned int)time(NULL));
    connect();
}

///	@param[in]	user	the User
void RevoltCPP::Revolt::on_ready(User) {
    log.debug("on_ready");
}

///	@param[in]	message	the Message that was received
void RevoltCPP::Revolt::on_message(Message) {
...
*/
void RevoltCPP::Revolt::on_user_ban(User, Guild) {
...
*/
void RevoltCPP::Revolt::on_user_unban(User, Guild) {
...
*/
void RevoltCPP::Revolt::on_user_join(Member, Guild) {
...
*/
void RevoltCPP::Revolt::on_user_remove(User, Guild) {
...
*/
void RevoltCPP::Revolt::on_typing_start(User, TextChannel, unsigned int) {
...
*/
void RevoltCPP::Revolt::update_presence(const std::string& status, Activity activity, const bool afk, const int shard_id) {
    json presence = {
        {"op", 3},  //
        {"d", {
                  {"since", {}},       //
...
std::shared_ptr<RevoltCPP::MainGateway> RevoltCPP::Revolt::get_shard(unsigned int shard_id) {
    for (unsigned int i = 0; i < _gateways.size(); i++) {
...
RevoltCPP::Guild* RevoltCPP::Revolt::get_guild(const std::string& guild_id) {
...
void RevoltCPP::Revolt::on_websocket_incoming_message(const json& payload) {
...
void RevoltCPP::Revolt::handle_raw_event(const std::string& event_name, const json& data) {
