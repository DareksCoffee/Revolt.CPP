...
RevoltCPP::VoiceChannel::VoiceChannel(Revolt* client, const json& data, const std::string& token)
...
RevoltCPP::VoiceChannel::VoiceChannel(Revolt* client, const std::string& id, const std::string& token) {
    _token = token;
    std::string url = "/channels/" + id;
    *this = VoiceChannel(client, api_call(url), token);
}

RevoltCPP::VoiceChannel::VoiceChannel(const VoiceChannel& old)
    : RevoltCPP::Channel(old) {
    bitrate = old.bitrate;
...
RevoltCPP::VoiceChannel::~VoiceChannel() {
    delete parent;
    delete guild;
}
...
**/
std::shared_ptr<RevoltCPP::VoiceClient> RevoltCPP::VoiceChannel::connect() {
    ...
    Logger("revolt.voicechannel").debug("Payload with Opcode 4 (Gateway Voice State Update) has been sent");

    while (true) {
        for (auto voice_state : _client->_voice_states) {
            if ((voice_state->channel_id == this->id) && (voice_state->endpoint.length() > 1)) {
                Logger("revolt.voicechannel").debug("Creating new voice client.");
