...
std::string RevoltCPP::MainGateway::decompress_message(
...
json RevoltCPP::MainGateway::get_heartbeat_payload() {
    return {
        {"op", 1},
        {"d", ((_sequence_number == 0) ? "null" : std::to_string(_sequence_number))}};
}

void RevoltCPP::MainGateway::on_websocket_incoming_message(
    const std::string& message) {
...
std::shared_future<void> RevoltCPP::MainGateway::send_heartbeat_ack() {
...
void RevoltCPP::MainGateway::identify() {
...
RevoltCPP::MainGateway::MainGateway(const std::string& token,
    ...
    _log =
        Logger("Revolt.MainGateway (shard id: [" + std::to_string(shard_id) +
...
unsigned int RevoltCPP::MainGateway::get_shard_id() {
    return _shard_id;
}

std::shared_future<void> RevoltCPP:MainGateway::connect(const std::string& url) {
    zs.zalloc = Z_NULL;
    zs.zfree = Z_NULL;
    zs.opaque = Z_NULL;
    zs.avail_in = 0;
