...
RevoltCPP::TextChannel::TextChannel(const json& data, const std::string& token) : RevoltCPP::Channel(data, token) {
...
RevoltCPP::TextChannel::TextChannel(const std::string& id, const std::string& token) {
    _token = token;
    std::string url = "/channels/" + id;
    *this = TextChannel(api_call(url), token);
}

RevoltCPP::TextChannel::TextChannel(const TextChannel& old) : RevoltCPP::Channel(old) {
    topic = old.topic;
    nsfw = old.nsfw;
