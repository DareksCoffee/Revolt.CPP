...
RevoltCPP::udp_client::udp_client(const std::string& ip, const int port) {
    _log = Logger("revolt.VoiceClient.udp_client");
...
RevoltCPP::udp_client::~udp_client() {
    _log.debug("~udp_client");
    _socket->close();
}

void RevoltCPP::udp_client::send(const std::string& msg) {
    try {
...
std::string RevoltCPP::udp_client::receive() {
    size_t len = 0;
...
std::shared_future<void> RevoltCPP::VoiceClient::connect_voice_udp() {
...
std::shared_future<void> RevoltCPP::VoiceClient::select_protocol() {
    return threadpool->execute([this] {
        json payload = {
            {"op", 1},
            {"d", {
...
RevoltCPP::VoiceClient::VoiceClient(std::shared_ptr<MainGateway> main_ws,
...
RevoltCPP::VoiceClient::~VoiceClient() {
...
std::shared_future<void> RevoltCPP::VoiceClient::disconnect() {
    if (!_ready) {
...
std::shared_future<void> RevoltCPP::VoiceClient::play(AudioSource* source) {
