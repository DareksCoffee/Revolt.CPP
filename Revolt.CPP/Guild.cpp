...
RevoltCPP::Guild::Guild(Revolt* client, const json& data, const std::string& token) : RevoltCPP::RevoltObject(token) {
...
RevoltCPP::Guild::Guild(Revolt* client, const std::string& id, const std::string& token) : RevoltCPP::RevoltObject(token) {
    std::string url = "/guilds/" + id;

    *this = Guild(client, api_call(url), token);
}

RevoltCPP::Guild::Guild(const Guild& old) {
    _token = old._token;
    id = old.id;
    name = old.name;
    icon = old.icon;
...
void RevoltCPP::Guild::_add_channel(Channel* channel) {
    channels.push_back(channel);
}

void RevoltCPP::Guild::_update_channel(Channel* channel) {
...
void RevoltCPP::Guild::_remove_channel(const std::string& channel_id) {
    for (size_t i = 0; i < channels.size(); i++) {
...
void RevoltCPP::Guild::_add_member(Member* member) {
    members.push_back(member);
    member_count += 1;
}

void RevoltCPP::Guild::_update_member(Member* member) {
...
void RevoltCPP::Guild::_remove_member(const std::string& member_id) {
    for (size_t i = 0; i < members.size(); i++) {
...
///@throws HTTPError
void RevoltCPP::Guild::leave() {
    std::string url = "/guilds/@me/guilds/" + id;
    api_call(url, "DEL");
}

///@throws HTTPError
void RevoltCPP::Guild::delete_guild() {
    std::string url = "/guilds/" + id;
    api_call(url, "DEL");
}
...
/**	@param[in]	user	User to kick
	@throws HTTPError
*/
void RevoltCPP::Guild::kick(const User& user) {
...
/**	@param[in]	user				User to ban
	...
	@throws HTTPError
*/
void RevoltCPP::Guild::ban(const User& user, const std::string& reason, const int delete_message_days) {
...
/**	@param[in]	user	User to kick
	@throws HTTPError
*/
void RevoltCPP::Guild::unban(const User& user) {
