...
RevoltCPP::Server::Server(Revolt* client, const json& data, const std::string& token) : RevoltCPP::RevoltObject(token) {
...
RevoltCPP::Server::Server(Revolt* client, const std::string& id, const std::string& token) : RevoltCPP::RevoltObject(token) {
    std::string url = "/servers/" + id;

    *this = Server(client, api_call(url), token);
}

RevoltCPP::Server::Server(const Server& old) {
    _token = old._token;
    id = old.id;
    name = old.name;
    icon = old.icon;
...
void RevoltCPP::Server::_add_channel(Channel* channel) {
    channels.push_back(channel);
}

void RevoltCPP::Server::_update_channel(Channel* channel) {
...
void RevoltCPP::Server::_remove_channel(const std::string& channel_id) {
    for (size_t i = 0; i < channels.size(); i++) {
...
void RevoltCPP::Server::_add_member(Member* member) {
    members.push_back(member);
    member_count += 1;
}

void RevoltCPP::Server::_update_member(Member* member) {
...
void RevoltCPP::Server::_remove_member(const std::string& member_id) {
    for (size_t i = 0; i < members.size(); i++) {
...
///@throws HTTPError
void RevoltCPP::Server::leave() {
    std::string url = "/servers/@me/servers/" + id;
    api_call(url, "DEL");
}

///@throws HTTPError
void RevoltCPP::Server::delete_server() {
    std::string url = "/servers/" + id;
    api_call(url, "DEL");
}
...
/**	@param[in]	user	User to kick
	@throws HTTPError
*/
void RevoltCPP::Server::kick(const User& user) {
    std::string url = "/servers/" + id + "/members/" + user.id;
    api_call(url, "DEL");
}
...
/**	@param[in]	user				User to ban
	...
	@throws HTTPError
*/
void RevoltCPP::Server::ban(const User& user, const std::string& reason, const int delete_message_days) {
    std::string url = "/servers/" + id + "/bans/" + user.id + "?delete-message-days=" + std::to_string(delete_message_days) + "&reason=" + urlencode(reason);
    api_call(url, "PUT");
}
...
/**	@param[in]	user	User to kick
	@throws HTTPError
*/
void RevoltCPP::Server::unban(const User& user) {
