...
RevoltCPP::RevoltObject::RevoltObject(const std::string& token) {
    _token = token;
...
void manage_cache() {
    std::thread([] {
        ...
        Logger log = Logger("revolt.object.manage_cache");
        while (1) {
...
/**	@param[in]	url				Relative url
	...
	@throws		HTTPError
*/
json RevoltCPP::RevoltObject::api_call(const std::string& url, const std::string& method, const json& data, const std::string& content_type, const bool cache) {
    if (method == "GET" && cache == true) {
        for (unsigned int i = 0; i < _cache.size(); i++) {
            if (_cache[i]->at("url").get<std::string>() == url) {
                Logger("revolt.object.api_call").debug("using cached result for: " + url);
    ...
    do {
        ...
        try {
            ...
            Logger("revolt.object.api_call").error("Error exception: " + std::string(e.what()));
        ...
        if (code == 429) {
            Logger("revolt.object.api_call").debug("Rate limit exceeded. Retry after: " + response.headers.at("Retry-After"));
    ...
    switch (code) {
        case 200:
            if (method == "GET") {
                ...
                Logger("revolt.object.api_call").debug("caching object");
            }
            break;
        case 400:
...
RevoltCPP::RevoltObject::http_response RevoltCPP::RevoltObject::request_internal(const std::string& target, const std::string& method, const std::string& data, const std::string& content_type) {
    Logger("revolt.object.request_internal").debug("sending message " + data + " via " + method + " to https://" + REVOLT_HOST + target);
    ...
    auto const results = resolver.resolve(REVOLT_HOST, "https");
    ssl::stream<tcp::socket> stream(io_context, ssl_context);
    boost::asio::connect(stream.lowest_layer(), results);

    if (!SSL_set_tlsext_host_name(stream.native_handle(), REVOLT_HOST))
    ...
    request.set(http::field::host, REVOLT_HOST);
    request.set(http::field::user_agent, "Revolt.CPP Revolt Bot");
    request.set(http::field::authorization, "Bot " + _token);
    request.set(http::field::connection, "Close");
    if (data.length() > 0) {
        Logger("revolt.object.request_internal").debug("setting message body \"" + data + "\" with content type \"" + content_type + "\" and content length \"" + std::to_string(data.length()) + "\"");
    ...
    Logger("revolt.object.request_internal").debug("response: " + std::to_string(response.result_int()) + ": " + response.body());
